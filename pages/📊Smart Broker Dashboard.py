...
st.subheader("Get Smart Broker GenAI based Recommendations")
...
if ask_question:
    question = f"use the dataframe with name df and columns ['Description/Amenities', Property BHK Type, 'Flat Size (sqft)', 'Price'] to generate pandas code " + variable \
        + 'For a string column of a dataframe, to apply a filter use df[column].str.lower().str.contains() and for integer column use df[column] >=.\
            In the filtered dataframe keep only columns [ID, Location, Developer, Flat Size (sqft), Property BHK Type, Price]. In case of only Price filter, no need to use other column. In case of no price filter, no need to use the column. Print the dataframe without the index values.'
